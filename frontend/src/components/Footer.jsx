      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            ...
            {settings.tagline && (
              <p className="text-xs text-store-primary font-medium mb-3">
                {settings.tagline}
              </p>
            )}
            ...
            <div className="flex flex-col gap-2 text-sm">
              <div className="flex items-center gap-2">
                <Mail className="h-4 w-4 text-store-primary" />
                <span>{settings.contactEmail || "support@shopkart.com"}</span>
              </div>
              <div className="flex items-center gap-2">
                <Phone className="h-4 w-4 text-store-primary" />
                ...
                <div className="flex gap-3 mt-4">
                  {settings.socialLinks?.instagram && (
                    <a
                      ...
                      className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-pink-600 hover:text-white transition-all"
                    >
                  ...
                  {settings.socialLinks?.twitter && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-sky-500 hover:text-white transition-all"
                    >
                      <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
            ...
            <ul className="space-y-2">
              {footerLinks.shop.map((link) => (
                <li key={link.name}>
                  <Link
                    to={link.to}
                    className="text-sm hover:text-store-primary transition-colors"
                  >
                    {link.name}
                  </Link>
                </li>
        ...
        <div className="border-t border-gray-800 py-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-sm text-gray-500">
            © {new Date().getFullYear()} {settings.storeName || "ShopKart"}. All
            rights reserved.
          </p>
